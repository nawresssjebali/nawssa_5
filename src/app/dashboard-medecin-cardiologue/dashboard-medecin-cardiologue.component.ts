import { Component, OnInit, Inject, PLATFORM_ID, AfterViewChecked, ViewChild, ElementRef, ViewEncapsulation, CUSTOM_ELEMENTS_SCHEMA, ChangeDetectorRef, QueryList, ViewChildren } from '@angular/core';
...
type?: 'text' | 'file' | 'voice' | 'voice_call' | 'video_call';
  ...
  formattedDuration?: string; 
     // Formatted duration string, e.g. "0:23" // propriété optionnelle pour contrôle d'affichage dans l'UI
}
...
export interface Case {
  ...
  selectedECGFile?: string | null;
   responseText?: string;
   answered?: string;
  answer?: string;
  responseFileName?: string;// add any other fields your template uses
   webPath?: string;
  votes: Vote[];
  choices: string[];
...
@Component({
  selector: 'app-dashboard-medecin-cardiologue',
  standalone: true,
  imports: [
  MatButtonModule,
  ...
  templateUrl: './dashboard-medecin-cardiologue.component.html',
styleUrls: ['./dashboard-medecin-cardiologue.component.css'],

  encapsulation: ViewEncapsulation.None ,// Corrected: Added a comma here
  schemas: [CUSTOM_ELEMENTS_SCHEMA]
})

export class DashboardMedecinCardiologueComponent implements OnInit, AfterViewChecked {
@ViewChild('localVideo') localVideoRef!: ElementRef<HTMLVideoElement>;
@ViewChild('remoteVideo') remoteVideoRef!: ElementRef<HTMLVideoElement>; @ViewChild('remoteAudio') remoteAudioRef!: ElementRef<HTMLAudioElement>;
@ViewChild('chatBox') chatBox!: ElementRef;
@ViewChildren('audioRef') audioRefs!: QueryList<ElementRef<HTMLAudioElement>>;
cases_1:BroadcastCase[] = [];
isLoadingCases = true;
selectedVotes: { [caseId: string]: string } = {};
tempResponses: { [ecgId: string]: string } = {};
   allReports: any[] = [];
    isRecording = false;
...
callerName: string | null = null;
 ...
 // set from your chat logic
  ...
  activeSection: string = 'oath' as const;
  isLoading: boolean = false;
  responseMessage: string | null = null;
  imagePreview: string | null = null;
  selectedDoctorId: string | null = null; 
 isLoadingDoctors: boolean = false;
 incomingCallerUserId: string | null = null; // add this to your component class

  generalDoctors: any[] = [];
   loading: boolean = false;
  ecgs: Ecg[] = [];
  apiBaseUrl: string = 'http://localhost:5000'; 
   // Cardiologist's ID, set elsewhere by selection
  senderId: string = '';          // Generalist user ID from localStorage
  receiverId: string = '';
...
currentCaseStep = -1;
 ...
 activeDoctor: string = '';
   ...
   selectedCase?: Case;
  // Toggle to show/hide full patient info
...
chatbotMessage: string = '';
  // Response text bound to form input via [(ngModel)]
responseText: string = '';
   // Tracks expanded case views
...
caseDataFinal: { [key: string]: string } = {}; // ✅ Store completed case before reset

// ✅ Tracks response text inputs per case
responseTexts: { [caseId: string]: string } = {};
...
unreadCounts: { [senderId: string]: number } = {};
 // your voting model
  ...
  selectedEmergencyLevel: string = ''; // empty = show all
  emergencyLevels: string[] = ['', 'normal', 'urgent', 'critical'];
 ...
 private route: ActivatedRoute,
  ...
  private cdr: ChangeDetectorRef  // <-- Ajouté ici
) {
  ...
  if (this.isBrowser) {
    ...
    if (userData) {
      try {
        ...
        console.log('Sender ID loaded from localStorage:', this.senderId);

        // ======= Notification permission check/request =======
  ...
  if (savedReport) {
    try {
      ...
      console.warn('⚠️ Failed to parse saved form data', e);
        ...
        if (this.receiverId) {
          this.fetchEcgsForCardiologist(this.senderId, this.receiverId);
        }
...
this.chatService.onMessagesRead().subscribe((data) => {
  ...
  console.log('Current conversations keys:', Object.keys(this.conversations));
  ...
  for (const roomId in this.conversations) {
    ...
    this.conversations[roomId].forEach((msg: any) => {
      console.log(`Message from senderId=${msg.senderId}, receiverId=${msg.receiverId}, status=${msg.status}`);

      if (
        msg.receiverId === readerId &&   // message is received by the reader
        msg.senderId !== readerId &&     // message sent by someone else
        msg.status !== 'read'
      ) {
        msg.status = 'read';
        updatedCount++;
        console.log(`Updated message status to 'read' in room ${roomId}`);
      }
    });
  }
  ...
  this.cdRef.detectChanges();
        ...
        // ===== RECEIVE MESSAGES (DO NOT mark as read here) =====
        this.chatService.receiveMessages().subscribe((message: any) => {
          console.log('🔔 Received message:', message);

          if (message.file && !message.file.type && message.file.name && message.file.name.endsWith('.webm')) {
          ...
          // Show notification if message is for this user and sender is someone else
  ...
  await this.handleIncomingCall(
    offer,
    socketId,
    callerUserId,              // correct slot
    callerName || 'Doctor',    // correct slot
    callType || 'voice'        // correct slot
  );
});
  ...
  console.log('📥 Call answer received from', socketId);
  if (this.peerConnection) {
    ...
    if (this.callTimeout) {
      ...
      console.log('✅ Cleared call timeout after receiving answer');
    }

    // ✅ ADD THIS
    this.callStatus = 'connected';
    this.isCalling = false;
    console.log('✅ Caller UI updated to connected');
  }
});
...
this.chatService.onCallRejected().subscribe(({ socketId }) => {
  ...
  // ❌ Stop and detach local video
  if (this.localVideoRef?.nativeElement?.srcObject) {
    (this.localVideoRef.nativeElement.srcObject as MediaStream).getTracks().forEach(track => track.stop());
    this.localVideoRef.nativeElement.srcObject = null;
    console.log('🛑 Local video tracks stopped on call rejection');
  }

  // ❌ Stop and detach remote video/audio
  if (this.remoteVideoRef?.nativeElement?.srcObject) {
    (this.remoteVideoRef.nativeElement.srcObject as MediaStream).getTracks().forEach(track => track.stop());
    this.remoteVideoRef.nativeElement.srcObject = null;
    console.log('🛑 Remote video tracks stopped on call rejection');
  }
  if (this.remoteAudioRef?.nativeElement?.srcObject) {
    (this.remoteAudioRef.nativeElement.srcObject as MediaStream).getTracks().forEach(track => track.stop());
    this.remoteAudioRef.nativeElement.srcObject = null;
    console.log('🛑 Remote audio tracks stopped on call rejection');
  }
  ...
  if (this.callTimeout) {
    clearTimeout(this.callTimeout);
    this.callTimeout = null;
    console.log('⏹️ Timeout cleared because call was rejected');
  }

  console.log('✅ Call fully cleaned up after rejection');
});
...
this.chatService.onCallCanceled().subscribe(({ socketId }) => {
  ...
  if (this.localVideoRef?.nativeElement?.srcObject) {
    (this.localVideoRef.nativeElement.srcObject as MediaStream).getTracks().forEach(track => track.stop());
    this.localVideoRef.nativeElement.srcObject = null;
    console.log('🛑 Local video tracks stopped on call cancel');
  }
  ...
  if (this.remoteVideoRef?.nativeElement?.srcObject) {
    (this.remoteVideoRef.nativeElement.srcObject as MediaStream).getTracks().forEach(track => track.stop());
    this.remoteVideoRef.nativeElement.srcObject = null;
    console.log('🛑 Remote video tracks stopped on call cancel');
  }
  if (this.remoteAudioRef?.nativeElement?.srcObject) {
    (this.remoteAudioRef.nativeElement.srcObject as MediaStream).getTracks().forEach(track => track.stop());
    this.remoteAudioRef.nativeElement.srcObject = null;
    console.log('🛑 Remote audio tracks stopped on call cancel');
  }
  ...
  console.log('✅ Incoming call canceled – fully cleaned up');
});


this.chatService.listenToCallReminders(
  (msg) => {
    console.log('📅 1h Reminder received:', msg);
    this.showReminderNotification('📅 Reminder', msg);
  },
  (msg, opts) => {
    console.log('⏰ 10m Reminder received:', msg);
    this.showReminderNotification('⏰ Call Alert', msg, opts?.sound);
        ...
        // Multilingual Speech Recognition Setup (unchanged)
...
fetchGeneralDoctors() {
    ...
    this.http.get<any[]>('http://localhost:5000/api/generalist-doctors')
      .subscribe({
        next: (doctors) => {
  ...
  console.log("Selected doctor object:", doctor);  // Log the doctor object
...
getDoctorId(doctor: any): string | null {
  console.log("Doctor object in getDoctorId:", doctor);  // Log input to helper function
  if (!doctor) return null;
  if (doctor.id) {
    return typeof doctor.id === 'string' ? doctor.id : doctor.id.$oid || null;
  }
  if (doctor._id) {
...
onRespondECG(event: Event, index: number, doctor: any): void {
  console.log('onRespondECG called');
  event.stopPropagation();
  this.activeSection = 'send-ecg';

  this.selectedDoctorId = this.getDoctorId(doctor);
  console.log('Selected doctor ID (sender):', this.selectedDoctorId);

  const loggedInUserId = this.user?.id;
  console.log('Logged-in cardiologist ID (receiver):', loggedInUserId);

  if (!loggedInUserId || !this.selectedDoctorId) {
    console.warn('⚠️ Missing IDs:', loggedInUserId, this.selectedDoctorId);
    return;
  }

  console.log('Sender ID:', this.selectedDoctorId);
  console.log('Receiver ID:', loggedInUserId);

  this.fetchEcgsForCardiologist(this.selectedDoctorId, loggedInUserId);
}

fetchEcgsForCardiologist(senderId: string, receiverId: string) {
  console.log('fetchEcgsForCardiologist called with sender:', senderId, 'receiver:', receiverId);
  ...
  const url = `${this.apiBaseUrl}/api/ecgs_naw?senderId=${encodeURIComponent(senderId)}&receiverId=${encodeURIComponent(receiverId)}`;
  console.log('Request URL:', url);

  this.isLoading = true;
  this.ecgs = [];  // reset before fetching

  this.http.get<any[]>(url).subscribe({
  next: (response) => {
    console.log('ECGs fetched:', response);

    // Map each ECG to add a usable webPath URL
    this.ecgs = response.map(ecg => {
      // Extract just the filename from filePath
      const filename = ecg.filePath.split('\\').pop();

      // Construct the public URL to your backend static files folder
      ecg.webPath = `${this.apiBaseUrl}/ecg-files/${filename}`;
...
submitResponse(ecg: any): void {
  const response = this.tempResponses[ecg._id];
  console.log("Response for ECG ID:", ecg._id, response);
  ...
  if (response && response.trim()) {
    ...
    const url = `${this.apiBaseUrl}/api/ecgs/respond`;
    ...
    console.error('Please write a response before submitting.');
  }
}
...
checkTodayEvents(events: any[]) {
  ...
  onRespondCase(event: Event, index: number, doctor: any): void {
  console.log('onRespondCase called');
  event.stopPropagation();
  ...
  this.fetchCases(this.selectedDoctorId, loggedInUserId);
}
fetchCases(senderId: string, receiverId: string) {
  if (!senderId || !receiverId) {
    console.error('Missing sender or receiver ID');
    return;
  }

  const url = `${this.apiBaseUrl}/api/cases_naw?senderId=${encodeURIComponent(senderId)}&receiverId=${encodeURIComponent(receiverId)}`;
  ...
  next: (response) => {
    ...
    this.cases = response;
    this.selectedCase = response.length > 0 ? response[0] : null;
    this.isLoading = false;
  },
  error: (err) => {
 ...
 toggleFullInfo(caseId: string) {
    this.expandedCases[caseId] = !this.expandedCases[caseId];
  }

  isFullInfoShown(caseId: string): boolean {
    return !!this.expandedCases[caseId];
  }

  handleResponseFile(event: Event, caseItem: any) {
    const input = event.target as HTMLInputElement;
    if (input.files && input.files.length > 0) {
      this.caseFiles[caseItem._id] = input.files[0];
    }
  }

  submitResponse_naw(caseItem: any) {
    const responseText = this.responseTexts[caseItem._id];

    if (!responseText || responseText.trim() === '') {
      console.warn(`Response for ECG ID: ${caseItem._id} is undefined`);
      alert('Please write a response before submitting.');
      return;
    }

    const formData = new FormData();
    formData.append('diagnosis', responseText);
    formData.append('caseId', caseItem._id);

    if (this.caseFiles[caseItem._id]) {
      formData.append('responseFile', this.caseFiles[caseItem._id]);
    }

    this.http.post('http://localhost:5000/submit-response', formData).subscribe({
      next: () => {
        alert('Response submitted successfully.');
        this.responseTexts[caseItem._id] = ''; // Clear text area
        delete this.caseFiles[caseItem._id];   // Remove attached file
      },
      error: (err) => {
        console.error(err);
...
loadECGs() {
  console.log('loadECGs called');
  this.isLoading_1 = true;

  // Retrieve current user ID from localStorage (assuming stored as JSON with 'id' field)
  const userData = localStorage.getItem('user');
  if (userData) {
    try {
  ...
  console.log('Fetching ECGs from backend...');
  // Fetch ECGs from backend
  this.http.get<Ecg[]>('http://localhost:5000/api/broadcast-ecgs').subscribe({
    next: (data) => {
      console.log(`Received ${data.length} ECG(s) from backend`);
      this.ecgs_1 = data;
      this.isLoading_1 = false;
    },
    error: (err) => {
      console.error('Error loading ECGs', err);
      this.isLoading_1 = false;
    }
  });
}
...
hasVoted(ecg: Ecg): boolean {
  if (!this.currentUserId) return false;
  return ecg.votes.some(vote => vote.cardiologistId === this.currentUserId);
}

getUserVote(ecg: Ecg): string | null {
  if (!this.currentUserId) return null;
  const vote = ecg.votes.find(v => v.cardiologistId === this.currentUserId);
  return vote ? vote.choice : null;
}

countVotes(ecg: Ecg, choice: string): number {
  return ecg.votes.filter(v => v.choice === choice).length;
}
...
submitVote(ecg: Ecg) {
  ...
  const selectedChoice = this.selectedVotes[ecg._id];
  if (!selectedChoice) return;

  const votePayload: VoteRequest = {
    ecgId: ecg._id,
    cardiologistId: currentUserId,
    choice: selectedChoice,
  };

  this.http.post('http://localhost:5000/api/broadcast-ecgs/vote', votePayload).subscribe({
    next: (response) => {
      ecg.votes.push({
        cardiologistId: currentUserId,
        choice: selectedChoice,
      ...
      delete this.selectedVotes[ecg._id];
    },
    error: (err) => {
...
onSendToAllDoctor() {
  this.activeSection = 'respond-broadcastecg';
  this.loadECGs();
}

selectCase(caseItem: any) {
  this.selectedCase = caseItem;
  this.showFullInfo = false;  // optionally reset the full info toggle on new selection
}
isVotingClosed(ecg: any): boolean {
  return ecg.answered === 'yes' || ecg.answered === 'expired';
}
// In dashboard-medecin-cardiologue.component.ts

isAnswerArray(ecg: any): boolean {
  return Array.isArray(ecg.answer);
}

getAnswerText(ecg: any): string {
  if (this.isAnswerArray(ecg)) {
    return ecg.answer.join(', ');
  } else {
    return ecg.answer;
  }
}
...
fullInfoShown: Set<string> = new Set();

getBroadcastedCases() {
  console.log('Calling GET /api/broadcast-cases_naw...');
  return this.http.get<any[]>('http://localhost:5000/api/broadcast-cases_naw');
}

loadCases() {
  console.log('Starting to load cases...');
  this.isLoadingCases = true;
  this.getBroadcastedCases().subscribe({
    next: (data) => {
...
// Switch to cases section and load data
onSendToAllDoctor_case() {
  console.log('Switching active section to respond-broadcastcase and loading cases');
  this.activeSection = 'respond-broadcastcase';
  this.loadCases();
}
hasVoted_1(caseItem: any): boolean {
  const voted = caseItem.votes?.some((v: any) => v.specialistId === this.currentUserId) || false;
  console.log(`hasVoted_1 for case ${caseItem._id}:`, voted);
  return voted;
}

getUserVote_1(caseItem: any): string {
  const vote = caseItem.votes?.find((v: any) => v.specialistId === this.currentUserId);
  console.log(`getUserVote_1 for case ${caseItem._id}:`, vote ? vote.choice : 'No vote found');
  return vote ? vote.choice : '';
}

countVotes_1(caseItem: any, choice: string): number {
  if (!caseItem.votes) {
    console.log(`countVotes_1 for case ${caseItem._id}, choice "${choice}": votes array is empty or undefined`);
    return 0;
  }
  const count = caseItem.votes.filter((v: any) => v.choice === choice).length;
  console.log(`countVotes_1 for case ${caseItem._id}, choice "${choice}":`, count);
  return count;
}

// Submit a vote for a specific case
submitVote_1(caseItem: any) {
  const userData = localStorage.getItem('user');
  if (userData) {
    try {
      const user = JSON.parse(userData);
  ...
  if (!this.selectedVotes[caseItem._id]) {
    console.log(`submitVote_1 aborted: no vote selected for case ${caseItem._id}`);
    return;
  }

  const votePayload = {
    specialistId: this.currentUserId,
    choice: this.selectedVotes[caseItem._id]
  };

  console.log(`Submitting vote for case ${caseItem._id}:`, votePayload);

  this.http.post(`http://localhost:5000/api/broadcast-cases/${caseItem._id}/vote`, votePayload)
    .subscribe({
      next: (updatedCase: any) => {
        console.log(`Vote submitted successfully for case ${caseItem._id}`, updatedCase);
        // Update the case locally with the new votes array
        const index = this.cases_1.findIndex(c => c._id === updatedCase._id);
        if (index > -1) {
          this.cases_1[index] = updatedCase;
        }
        // Optionally clear the selected vote
        // this.selectedVotes[caseItem._id] = '';
      },
      error: (err) => {
...
ffetchEvents(): void {
  ...
  this.http.get<CalendarBookingEvent[]>(`http://localhost:5000/events?userId=${userId}`).subscribe({
    next: (events) => {
      ...
      // ✅ Put this line here to ensure all events have participantNames initialized
      this.events = events.map(event => ({
        ...event,
        date: new Date(event.date),
        participantNames: [] // initialized to avoid 'possibly undefined' error
      }));
...
onFAQ() {
    ...
    this.http.get<any[]>('http://localhost:5000/api/faqs_2').subscribe(
      data => {
  ...
  if (trimmedQuestion) {
    ...
    this.http.post('http://localhost:5000/api/faqs_2', body).subscribe({
      next: response => {
...
deleteEvent(event: CalendarBookingEvent): void {
 ...
 get filteredEcgs() {
  if (!this.selectedEmergencyLevel) {
    return this.ecgs_1;
  }
  return this.ecgs_1.filter(ecg => ecg.emergencyLevel === this.selectedEmergencyLevel);
}
selectedEmergencyLevel_1: string = '';

get filteredEcgs_1() {
  if (!this.selectedEmergencyLevel_1) return this.ecgs;
  return this.ecgs.filter(ecg => ecg.emergencyLevel === this.selectedEmergencyLevel_1);
}
}